 ...
 * authenticated user - users can only access their own projects.
 *
 ...
 *   <li>400 Bad Request - Validation failed (e.g., duplicate project name, project doesn't belong to user)</li>
 * </ul>
 *
 * @see ProjectService
...
public class ProjectController {
     ...
     * </pre>
     *
     * <p><strong>Required Fields:</strong> name
     * <p><strong>Optional Fields:</strong> description, status (defaults to PLANNING)
     *
     * <p><strong>Note:</strong> The project is automatically assigned to the authenticated user.
    ...
    public ResponseEntity<ProjectResponseDto> createProject(
        ...
        ProjectResponseDto createdProject = projectService.createProject(createDto, userId);
     ...
     *
     * <p><strong>Note:</strong> Only returns the project if it belongs to the authenticated user.
     *
     * <p><strong>Status Codes:</strong>
     * <ul>
     *   <li>200 OK - Project found and returned</li>
     *   <li>400 Bad Request - Project doesn't belong to authenticated user</li>
    ...
    public ResponseEntity<ProjectResponseDto> getProjectById(
        ...
        Long userId = userDetails.getAppUser().getId();
        ProjectResponseDto project = projectService.getById(id, userId);
     ...
     * <p><strong>Path:</strong> /api/projects
     *
     * <p><strong>Note:</strong> Always returns only projects belonging to the authenticated user.
     *
     ...
     * <ul>
     *   <li>No params: Returns all user's projects</li>
     *   <li>status only: User's projects with specific status</li>
     *   <li>name only: User's projects matching name search</li>
     * </ul>
     *
     * <p><strong>Status Codes:</strong>
     * <ul>
    ...
    public ResponseEntity<List<ProjectResponseDto>> getAllProjects(
        ...
        // Status filter
        if (status != null) {
            projects = projectService.findByStatus(status, userId);
            return ResponseEntity.ok(projects);
        }

        // Name search filter
        if (name != null) {
            projects = projectService.findByNameContaining(name, userId);
            return ResponseEntity.ok(projects);
        }

        // No filters - return all user's projects
     ...
     *   <li>400 Bad Request - Project name already exists for user, or project doesn't belong to user</li>
     *   <li>404 Not Found - Project with given ID doesn't exist</li>
     * </ul>
     *
     * @param userDetails the authenticated user (injected by Spring Security)
